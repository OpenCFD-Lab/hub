...
) const
{
    ...
    for (int procI=0; procI<Pstream::nProcs(); procI++) // by SBLEE
    {
...
) const
{
    ...
    while (true)
    {
        ...
        Pstream::listCombineGather(regionType, maxEqOp<label>()); // by SBLEE
        Pstream::listCombineScatter(regionType); // by CWKIM 
...
) const
{
    ...
    while (returnReduce(isFront.any(), orOp<bool>()))
    {
        ...
        DAUsyncTools::syncFaceList(mesh_, newIsFront, orEqOp<unsigned int>());
...
)
{
    ...
    while (true)
    {
        ...
        (
            ...
            greatPoint,                             // by SBLEE
            UPstream::msgType()
            //cellInterpolationMap().comm()  // by SBLEE
        );
    ...
    )
{
    ...
    Info<<"update inverseDistance : 1366"<<endl;
}
...
bool Foam::cellCellStencils::inverseDistance::update()
{
    ...
    scalar layerRelax(dict_.lookupOrDefault<scalar>("layerRelax", 1.0)); // by SBLEE

    //scalar tol = dict_.getOrDefault("tolerance", 1e-10);
    scalar tol = dict_.lookupOrDefault<scalar>("tolerance", 1e-10); // by SBLEE
    ...
    Pstream::listCombineGather(nCellsPerZone, plusEqOp<label>()); // by SBLEE
    Pstream::listCombineScatter(nCellsPerZone); // by CWKIM 
    ...
    // Determine zone meshes and bounding boxes
    {
        ...
        {
            fvMeshSubset* fvMeshSubsetPtr=new fvMeshSubset(mesh_); // by SBLEE
        ...
        Pstream::gatherList(procBb); // by SBLEE
        Pstream::scatterList(procBb); //by CWKIM
