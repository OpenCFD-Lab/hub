...
#include "DAUmeshToMeshMethod.H"
#include "DAUtetOverlapVolume.H"
#include "OFstream.H"
#include "Time.H"
...
namespace Foam
{
    defineTypeNameAndDebug(DAUmeshToMeshMethod, 0);
    defineRunTimeSelectionTable(DAUmeshToMeshMethod, components);
}

Foam::scalar Foam::DAUmeshToMeshMethod::tolerance_ = 1e-6;

// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

Foam::labelList Foam::DAUmeshToMeshMethod::maskCells() const
{
...
bool Foam::DAUmeshToMeshMethod::intersect
(
...
) const
{
    ...
    DAUtetOverlapVolume overlapEngine;
    ...
    DAUtreeBoundBox bbTgtCell(tgt_.points(), tgt_.faces()[cellFaces[0]]);
...
Foam::scalar Foam::DAUmeshToMeshMethod::interVol
(
    const label srcCelli,
    const label tgtCelli
) const
...
Foam::DAUmeshToMeshMethod::interVolAndCentroid
(
    const label srcCelli,
    const label tgtCelli
)
...
void Foam::DAUmeshToMeshMethod::appendNbrCells
(
    const label celli,
    const polyMesh& mesh,
...
) const
{
    ...
    for (const label nbrCelli : nbrCells)
    {
        ...
        // START : by SBLEE
        bool found=false;
        forAll(visitedCells,i)
        {
            if(visitedCells[i]==nbrCelli)
            {
                found=true;
                break;
            }
        ...
        // END : by SBLEE
        {
            ...
            forAll(nbrCellIDs, i)
            {
                if(nbrCellIDs[i]==nbrCelli) pos=i;
            }
            if(pos==-1) nbrCellIDs.append(nbrCelli);
            // END : by SBLEE
        }
    }
}
...
bool Foam::DAUmeshToMeshMethod::initialise
(
...
Foam::DAUmeshToMeshMethod::DAUmeshToMeshMethod
(
    const polyMesh& src,
    const polyMesh& tgt
)
...
void Foam::DAUmeshToMeshMethod::writeConnectivity
(
