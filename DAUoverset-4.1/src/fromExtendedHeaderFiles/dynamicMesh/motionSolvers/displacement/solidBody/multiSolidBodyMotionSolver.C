...
#include "bitSet.H"
#include "DAUsyncTools.H" // by SBLEE
    ...
    points0MotionSolver(mesh, dict, typeName)
{
    ...
    for (const entry& dEntry : coeffDict())
    {
        if (dEntry.isDict())
        {
            ...
            labelList zoneIDs = mesh.cellZones().findIndices(cellZoneName); // by SBLEE
            ...
            DAUsyncTools::syncPointList  // by SBLEE
            (
...
Foam::tmp<Foam::pointField> Foam::multiSolidBodyMotionSolver::curPoints() const
{
    ...
    auto ttransformedPts = tmp<pointField>(new pointField(mesh().points())); // by SBLEE
