...
void Foam::bitSet::set(const labelRange& range)
{
    ...
    //slice.adjust();  // No negative start, size adjusted accordingly // by SBLEE
    ...
    //if (slice.after() >= size())
    if (slice.last() >= size()) // by SBLEE
    {
        //reserve(slice.after());
        reserve(slice.last()); // by SBLEE
        //resize(slice.start(), false);
        resize(slice.first(), false); // by SBLEE
        //resize(slice.after(), true);
        resize(slice.last(), true); // by SBLEE
        return;
    }
    ...
    unsigned int eblock = slice.last() / elem_per_block; // by SBLEE
    //unsigned int emask  = slice.after() % elem_per_block;
    unsigned int emask  = slice.last() % elem_per_block; // by SBLEE
...
void Foam::bitSet::unset(const labelRange& range)
{
    ...
    // START - by SBLEE
    labelRange slice;
    const label lower = Foam::max(range.first(),0);
    const label upper = Foam::min(range.last(), Foam::max(0,size()-1));
    const label total = upper+1 - lower;
    if(total>0) slice=labelRange(lower,total);
    else slice=labelRange();
    // END - by SBLEE
