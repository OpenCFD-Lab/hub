...
:
    DAUcoupledFvPatchField<Type>(p, iF),
...
:
    DAUcoupledFvPatchField<Type>(ptf),
...
:
    DAUcoupledFvPatchField<Type>(ptf, iF),
    procInterface_(ptf.procInterface_),
    sendBuf_(procInterface_.faceCells().size()),
    receiveBuf_(procInterface_.faceCells().size()),
    scalarSendBuf_(procInterface_.faceCells().size()),
...
)
{
    if (Pstream::parRun())
    {
        ...
        (
            ...
            //this->data_bytes(),
            reinterpret_cast<char*>(this->data()),  // by SBLEE
            //this->size_bytes(),
            this->byteSize(),  // by SBLEE
            procInterface_.tag(),
            procInterface_.comm()
        );
        ...
        (
            ...
            reinterpret_cast<char*>(sendBuf_.data()),  // by SBLEE
            //sendBuf_.size_bytes(),
            sendBuf_.byteSize(),  // by SBLEE
            procInterface_.tag(),
            procInterface_.comm()
        );
    }
...
//4.1ver
    if (Pstream::parRun())
    {
        ...
        {
            procInterface_.compressedSend(commsType, sendBuf_);
        }
    }
*/
}
...
/*  //4.1
    if (Pstream::parRun())
    {
        ...
        {
            procInterface_.compressedReceive<Type>(commsType, *this);
        }

        if (doTransform())
        {
            transform(*this, procInterface_.forwardT(), *this);
        }
    }
*/
}
...
(
    ...
    const scalarField& coupleCoeffs,
    const direction cmpt,
    const Pstream::commsTypes commsType
) const
{
        const labelUList& fC = this->procInterface_.faceCells();
        scalarSendBuf_.setSize(fC.size());

        forAll(fC, i)
            scalarSendBuf_[i] = psiif[fC[i]];
...
) const
{
    ...
    addToInternalField(result, false, coupleCoeffs, scalarReceiveBuf_);
