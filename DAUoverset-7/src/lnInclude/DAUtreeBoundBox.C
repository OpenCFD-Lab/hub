...
/*
const Foam::faceList Foam::DAUtreeBoundBox::faces
({
...
// END - by SBLEE
/*
const Foam::edgeList Foam::DAUtreeBoundBox::edges
({
    {0, 1}, // 0
    {1, 3},
    {2, 3}, // 2
...
Foam::DAUtreeBoundBox::DAUtreeBoundBox(const UList<point>& points)
:
    DAUboundBox(points, false)
{
    if (points.empty())
    {
        WarningInFunction
...
Foam::DAUtreeBoundBox::DAUtreeBoundBox
(
    const UList<point>& points,
    const labelUList& indices
)
:
    DAUboundBox(points, indices, false)
{
...
Foam::tmp<Foam::pointField> Foam::DAUtreeBoundBox::points() const
{
    //auto tpts = tmp<pointField>::New(8);
    auto tpts = tmp<pointField>(new pointField(8)); // by SBLEE
    auto& pts = tpts.ref();
...
Foam::DAUtreeBoundBox Foam::DAUtreeBoundBox::subBbox(const direction octant) const
{
    return subBbox(centre(), octant);
}


Foam::DAUtreeBoundBox Foam::DAUtreeBoundBox::subBbox
(
    const point& mid,
...
) const
{
    ...
    DAUtreeBoundBox subBb(*this);
    point& bbMin = subBb.min();
    point& bbMax = subBb.max();

    if (octant & DAUtreeBoundBox::RIGHTHALF)
    {
    ...
    if (octant & DAUtreeBoundBox::TOPHALF)
    {
    ...
    if (octant & DAUtreeBoundBox::FRONTHALF)
    {
...
bool Foam::DAUtreeBoundBox::intersects
(
...
bool Foam::DAUtreeBoundBox::contains(const vector& dir, const point& pt) const
{
...
Foam::direction Foam::DAUtreeBoundBox::faceBits(const point& pt) const
{
    direction octant = 0;
...
Foam::direction Foam::DAUtreeBoundBox::posBits(const point& pt) const
{
    direction octant = 0;
...
void Foam::DAUtreeBoundBox::calcExtremities
(
    const point& pt,
    point& nearest,
    point& furthest
...
Foam::scalar Foam::DAUtreeBoundBox::maxDist(const point& pt) const
{
    point near, far;
...
Foam::label Foam::DAUtreeBoundBox::distanceCmp
(
    const point& pt,
    const DAUtreeBoundBox& other
) const
{
    //
...
Foam::Ostream& Foam::operator<<(Ostream& os, const DAUtreeBoundBox& bb)
{
    return os << static_cast<const DAUboundBox&>(bb);
}


Foam::Istream& Foam::operator>>(Istream& is, DAUtreeBoundBox& bb)
{
    return is >> static_cast<DAUboundBox&>(bb);
}
