...
Foam::DAUPackedList<Width>::DAUPackedList
(
    const DAUPackedList<Width>& list,
    const labelUList& addr
)
:
    DAUPackedList<Width>(addr.size())
{
...
:
    DAUPackedList<Width>(range.size())
{
    //label pos = range.start();
    label pos = range.first(); // by SBLEE
...
template<unsigned Width>
bool Foam::DAUPackedList<Width>::uniform() const
{
    // Trivial cases
    if (empty())
    {
...
template<unsigned Width>
bool Foam::DAUPackedList<Width>::equal(const DAUPackedList<Width>& other) const
{
    if (size() != other.size())
    {
        return false;
...
template<unsigned Width>
Foam::labelList Foam::DAUPackedList<Width>::values() const
{
...
Foam::List<IntType>
Foam::DAUPackedList<Width>::unpack() const
{
    static_assert
    (
...
template<class IntType>
Foam::List<IntType>
Foam::DAUPackedList<Width>::unpack(const labelRange& range) const
{
    static_assert
    (
        std::is_integral<IntType>::value,
...
template<class IntType>
Foam::List<IntType>
Foam::DAUPackedList<Width>::unpack(const labelUList& locations) const
{
    static_assert
    (
        std::is_integral<IntType>::value,
