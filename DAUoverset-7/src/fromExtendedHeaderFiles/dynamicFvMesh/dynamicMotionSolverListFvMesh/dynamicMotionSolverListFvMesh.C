    ...
    if (dict.found("solvers"))
    {
        ...
        for (const entry& dEntry : solvertDict)
        {
            if (dEntry.isDict())
            {
                ...
                io.readOpt()=IOobject::NO_READ; // by SBLEE
                //io.writeOpt(IOobject::AUTO_WRITE);
                io.writeOpt()=IOobject::AUTO_WRITE; // by SBLEE
...
bool Foam::dynamicMotionSolverListFvMesh::update()
{
    if (motionSolvers_.size())
    {
        ...
        */
        ...
        {
            volVectorField& Uref = const_cast<volVectorField&>(lookupObject<volVectorField>("U")); // by SBLEE
