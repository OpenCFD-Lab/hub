...
void Foam::DAUtetOverlapVolume::tetTetOverlap
(
...
{
    static tetIntersectionList insideTets;
    label nInside = 0;
    static tetIntersectionList cutInsideTets;
    label nCutInside = 0;
    ...
    for (label facei = 0; facei < 4; ++facei)
    {
        ...
        tetAFaceAreas[facei] = -tetATet.tri(facei).normal(); // by SBLEE
    ...
    for (label facei = 0; facei < 4; ++facei)
    {
        ...
        tetBFaceAreas[facei] = -tetBTet.tri(facei).normal(); // by SBLEE
    ...
    // Face 0
    {
        ...
        plane pl0(tetBTet.tri(0).a(), n); // by SBLEE

        // tetA.tet().sliceWithPlane(pl0, cutInside, outside);
        sliceWithPlane(tetA.tet(), pl0, cutInside, outside); // by SBLEE
        if (nCutInside == 0)
        {
            return;
        }
    ...
    // Face 1
    {
        ...
        plane pl1(tetBTet.tri(1).a(), n); // by SBLEE
        ...
        for (label i = 0; i < nCutInside; i++)
        {
            ...
            sliceWithPlane(t, pl1, inside, outside); // by SBLEE
        }
        if (nInside == 0)
        {
            return;
    ...
    // Face 2
    {
        ...
        plane pl2(tetBTet.tri(2).a(), n); // by SBLEE
        ...
        for (label i = 0; i < nInside; i++)
        {
            ...
            sliceWithPlane(t, pl2, cutInside, outside); // by SBLEE
        }
        if (nCutInside == 0)
        {
            return;
    ...
    // Face 3
    {
        ...
        plane pl3(tetBTet.tri(3).a(), n); // by SBLEE
        for (label i = 0; i < nCutInside; i++)
        {
            const tetPointRef t = cutInsideTets[i].tet();
            // t.sliceWithPlane(pl3, insideOp, outside);
            sliceWithPlane(t, pl3, insideOp, outside); // by SBLEE
        }
    }
}
...
void Foam::DAUtetOverlapVolume::cellCellOverlapMinDecomp
(
...
)
{
    ...
    forAll(cFacesA, cFA)
    {
        ...
        const DAUtreeBoundBox pyrA = pyrBb(meshA.points(), fA, ccA);
        ...
        for (label tetPtI = 1; tetPtI < fA.size() - 1; tetPtI++)
        {
            ...
            forAll(cFacesB, cFB)
            {
                ...
                const DAUtreeBoundBox pyrB = pyrBb(meshB.points(), fB, ccB);
